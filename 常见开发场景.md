...
### 不使用RNAbility的场景及注意事项

- 场景

  有些工程是原生页面跳转到RN页面的工程，不需要在EntryAbility继承RNAbility，所以这种场景就需要开发者自己实现RNAbility。

- 方案

  主要是重新实现RNAbility的onCreate中的方法，在项目初始化的时候执行。

- 注意事项

  需要在初始化的时候监听onWindowSizeChange方法设置正确的size，否则会出现字体非常小的情况。
